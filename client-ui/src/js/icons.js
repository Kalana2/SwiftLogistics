...
// Each icon is a mini SVG path. Lucide CDN is optional — these work standalone.
 ...
 * @param {number} size - Size in px (default 18)
...
export function icon(name, size = 18, cls = '') {
    ...
    if (!paths) {
        // Fallback: empty circle if icon not found
        return `<svg class="icon ${cls}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/></svg>`;
    }
...
/**
 * No-op — icons are already inline SVG, no replacement needed
 */
export function renderIcons() {
    // Icons are inline SVG — nothing to replace
}
